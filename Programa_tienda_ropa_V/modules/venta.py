...
def venta(self,controller_menu):
    ...
    self.btn_combinar=CTkButton(self.side_bar,
        font=config.Font3,  
        command=lambda: combinar(self),                
        text="Combinar medio de pago",
        text_color=config.Color_1,
        fg_color="#8EFF47",
        width=30,height=50,
        anchor="center",
    ...
    self.btn_combinar.pack(pady=10, padx=10)
    ...
    self.btn_mostrar_img.pack(pady=5, padx=10)
    img_producto(self)
    ...
    def realizar_venta():
       if self.tree_products.get_children():
           ...
           for cod, producto in productos_agrupados.items():
               ...
               if self.descuento > 0:
                   descripcion = f"color: {color} talle: {talle} marc: {marca}  desc: {self.descuento}"
               else:
               ...
               if self.optionmenu_tarjetas.get() != 'Ninguna': 
                    if self.entry_metodo_pago.get()=="Combinar":
                        metodo_pago=str(self.medios_combinados)
                    else:
                        metodo_pago += f" {self.optionmenu_tarjetas.get()}"
                    
                
    
               register(self,self.user_name, nom, descripcion, cantidad, "Ventas", prec_com*int(cantidad), metodo_pago)
